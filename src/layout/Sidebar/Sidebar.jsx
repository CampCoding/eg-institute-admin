...
//   return (
//     <div
//       className={`fixed max-h-[100vh] top-0 bottom-0 inset-y-0 left-0 z-50 !w-80 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
...
//         {/* Nav */}
//         <nav className="side-list px-4 py-6 space-y-2 overflow-y-auto">
//           {links.map((item) => {
...
const Sidebar = ({ colors, sidebarOpen, setSidebarOpen }) => {
  ...
  // افتح المنيو اللي ليها صفحة active
  useEffect(() => {
    if (!activeParentIds.size) return;
    setOpenMenus((prev) => {
      const next = { ...prev };
  ...
  return (
    ...
    >
      <div className="flex flex-col w-full h-full">
        ...
        {/* Nav (scrollable) */}
        <nav className="side-list px-4 py-6 space-y-2 overflow-y-auto flex-1">
          {links.map((item) => {
            const hasChildren = !!item.children?.length;
            const open = hasChildren && isMenuOpen(item);
            const isParentActive = parentActive(item);
