...
export default function InstructorProfile({ instructor }) {
  ...
  const data =
    instructor || {
      ...
      hourlyRate: 25,
      bio:
        "Dedicated Arabic educator focusing on communicative approaches for non-native learners. Passionate about pronunciation, rhythm, and confidence-building.",
      rating: 4.8,
      satisfaction: 96,
      studentsCount: 124,
      classesCount: 18,
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              ...
              <div className="text-xl font-semibold text-gray-900">{data.studentsCount}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              <div className="text-xs text-gray-500">Classes</div>
              <div className="text-xl font-semibold text-gray-900">{data.classesCount}</div>
            </div>
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              <div className="text-xs text-gray-500">Satisfaction</div>
              <div className="text-xl font-semibold text-gray-900">{data.satisfaction}%</div>
              <div className="mt-2"><Progress value={data.satisfaction} /></div>
            </div>
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              <div className="text-xs text-gray-500">Lessons Taught</div>
              <div className="text-xl font-semibold text-gray-900">{data.lessonsTaught}</div>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="divide-y">
                {data.courses.map((c) => (
                  <div key={c.id} className="py-4 grid grid-cols-12 gap-4 items-center">
                    <div className="col-span-6">
                      <div className="font-medium text-gray-900">{c.title}</div>
                      <div className="text-xs text-gray-500">Level {c.level} • ID: {c.id}</div>
                      <div className="text-xs text-gray-500 mt-1">Next: {c.nextLesson}</div>
                    </div>
