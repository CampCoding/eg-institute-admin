...
export default function AllStoreData() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Tab Navigation */}
      <div className="mt-5 !max-3-xl pb-3 sm:!pb-0 !overflow-x-auto px-2 sm:px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProducts.map((product) => (
            <StoreCard
              key={product.id}
              p={product}
              PRIMARY="#02AAA0" // Primary color for the card
              PRIMARY_DARK="#0E5F5B" // Darker primary color for hover effects
              PRIMARY_LIGHT="#23C7BD" // Lighter primary color for hover effects
            />
          ))}
        </div>
      </div>
