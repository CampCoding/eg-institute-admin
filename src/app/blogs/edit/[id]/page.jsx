...
import { Loader2, Save, ArrowLeft, Upload, Star, Tag as TagIcon } from "lucide-react";
...
export default function EditBlogPage() {
  ...
  const [form, setForm] = useState({
    ...
    tag: TAGS[3], // "None"
    readMins: 6,
    author: teachers?.[0]?.name ?? "Unknown",
  ...
  useEffect(() => {
    try {
      ...
      const source = fromDrafts
        ? fromDrafts
        : SEED_BLOGS.find((b) => b.id === paramKey || b.slug === paramKey) || null;
        ...
        setForm({
          ...
          tag:
            typeof source.tag === "string"
              ...
              : "None",
          readMins:
            typeof source.readMins === "number" ? source.readMins : 6,
          author: source.author?.name || source.author || teachers?.[0]?.name || "Unknown",
          date: (source.date || new Date().toISOString().slice(0, 10)).slice(0, 10),
          cover: source.cover || "",
          coverFileName: "",
  ...
  const levelBadgeClass = (v) => {
    ...
    if (x === "beginner") return "bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200";
    if (x === "intermediate") return "bg-blue-100 text-blue-700 ring-1 ring-blue-200";
    if (x === "advanced") return "bg-rose-100 text-rose-700 ring-1 ring-rose-200";
    if (x === "featured") return "bg-amber-200 text-amber-900 ring-1 ring-amber-300";
    if (x === "trending") return "bg-pink-200 text-pink-900 ring-1 ring-pink-300";
    if (x === "new") return "bg-violet-100 text-violet-700 ring-1 ring-violet-200";
        ...
        <form onSubmit={handleSubmit} className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl bg-white border border-slate-200 p-5">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <label className="text-sm font-medium">Arabic Title (optional)</label>
                  <input
                    value={form.title_ar}
                ...
                <div>
                  ...
                  <select
                    value={typeof form.author === "string" ? form.author : form.author?.name}
            ...
            <div className="rounded-2xl bg-white border border-slate-200 p-5">
              ...
              <p className="text-sm text-slate-600 mt-1">
                Upload an image or paste a URL. A soft gradient overlay is applied in the card.
              </p>
            ...
            <div className="rounded-2xl bg-white border border-slate-200 p-5">
              ...
              <p className="text-sm text-slate-600 mt-1">Write or update the main article body.</p>
              <textarea
                value={form.content}
          ...
          <div className="lg:sticky lg:top-24">
            <div className="rounded-2xl bg-white border border-slate-200 p-5">
              ...
              <p className="text-sm text-slate-600">Matches your blogs grid card.</p>
              ...
              >
                ...
                <div className="relative h-44 sm:h-56 overflow-hidden">
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-r ${form.gradient} opacity-25`} />
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-600">
                    <div className="inline-flex items-center gap-2">
                      <div className="h-8 w-8 rounded-full bg-slate-200 grid place-items-center text-[10px] font-semibold">
                        {(typeof form.author === "string" ? form.author : form.author?.name || "A")[0]}
                      </div>
                      <div className="flex flex-col leading-tight">
                        <span className="font-medium">
                          {typeof form.author === "string" ? form.author : form.author?.name || "Author"}
                        </span>
                        {form.category && (
                          <span className="text-[11px] text-slate-500">{form.category}</span>
                        )}
                      </div>
                    </div>
