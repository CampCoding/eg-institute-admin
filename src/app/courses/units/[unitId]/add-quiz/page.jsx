...
export default function AddQuizPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.quizMode === "upload") {
        const uploaded = await uploadPdf(data.quizFile); // نفس uploadPdf بتاع الـ PDFs
      ...
      if (res?.status === "success") {
        toast.success(res.message || "Quiz added");
        router.back();
      } else {
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <BreadCrumb title="Add Quiz" child="Quizzes" parent="Add Quiz" />
